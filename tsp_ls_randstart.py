...
# passing arguments as tuple because easier to work with map()
def hill_climb(args):
    points = args[0]
    restart = args[1]
    startTime = time.time()
    ...
    while True:
        if time.time() - startTime > 300:
            print("---hill climb with random restarts exceeds 5 minutes---")
            return curr, cost, steps, time.time()-startTime
        new_cost, next = best_neighbor(curr)
        if cost <= new_cost:
            if restart > 0:
                repath, recost, resteps, pasttime = hill_climb((points, restart-1))
                if recost < cost:
                    cost = recost
                    curr = repath
                    steps = resteps
